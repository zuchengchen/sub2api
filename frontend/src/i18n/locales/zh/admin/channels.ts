export default {
    ...
    riskControl: {
      ...
      usagePolicy: {
        ...
        summary: '统计 OpenAI 返回的 usage policy 拒绝，并在用户触发后立即禁用该用户。管理员账号不会被自动禁用。',
        enabled: '启用统计与自动处置',
        autoBan: '触发后立即禁用用户',
        autoBanHint: '达到阈值后将用户状态改为禁用，并刷新认证缓存。默认一次即封。',
        banThreshold: '封禁触发次数',
        total: '违规次数',
        uniqueUsers: '涉及用户',
        disabledUsers: '已禁用用户',
        autoBannedUsers: '自动封禁',
        empty: '暂无 usage policy 违规记录。',
        user: '用户',
        count: '次数',
        lastAt: '最近一次',
        status: '状态',
        ...
        statusDisabled: '已禁用',
        autoBanned: '已自动封禁',
        skippedAdmin: '管理员，跳过封禁',
        loadFailed: '加载 usage policy 统计失败',
        saveFailed: '保存 usage policy 配置失败',
      },
      loadFailed: '加载风控中心失败',
      saveFailed: '保存风控配置失败',
      logsFailed: '加载审核记录失败',
      ...
      refreshStatus: '刷新状态',
      reviewersTitle: '审核器',
      reviewersSummary: '线上审核池按 DS、Qwen、GLM、MiMo 顺序延迟并发；明确违规优先生效，策略限制需两家供应商一致。',
      onlineReviewersTitle: '线上审核池（DS / Qwen / GLM / MiMo）',
      onlineReviewersMeta: '无推理、纯文本审核；策略阻断需要两家独立供应商确认（阈值 {threshold}）。',
      ...
      restrictionConsensusReady: '策略阻断双重确认可用',
      restrictionConsensusUnavailable: '策略阻断确认需要 {required} 家独立供应商（当前 {reachable} 家）',
      ...
      reviewAttempts: '渠道尝试',
      reviewOutcome: '审核结论',
      reviewConsensus: '复审共识',
      attemptCount: '{count} 次尝试',
      enabledChannelRequired: 'DeepSeek 开启时至少需要启用一个渠道',
      twoRemoteReviewersRequired: '策略阻断确认至少需要两个可用的独立供应商',
      confirmedVoteCount: '{count} 家独立供应商一致限制',
      singleVoteDetail: '仅 1 家供应商限制，未形成策略共识',
      consensusUnknownDetail: '缺少双重确认凭据，不能视为已确认共识',
      undeterminedNonViolation: '未定性，不计违规',
      ...
      result: {
        ...
        blocked: '违规拦截',
        restricted: '策略阻断（非违规）',
        restrictedConfirmed: '策略阻断（双重确认，非违规）',
        restrictedUnconfirmed: '策略阻断（共识未知，非违规）',
        singleVoteUndetermined: '复审票数不足（单票，未定性）',
        disagreementUndetermined: '复审分歧（未定性）',
        unavailableUndetermined: '复审不可用（未定性）',
        evidenceCapacityUndetermined: '证据容量超限（未定性）',
        evidenceTruncatedUndetermined: '证据不完整（未定性）',
        pass: '未命中',
        error: '异常',
        unavailable: '审核不可用',
