export default {
    settings: {
      ...
      guideEditor: {
        title: '使用教程编辑器',
        description: '每一章都可以单独编辑。左边选章、右边改内容，保存后所有域名的 /guide 页面立刻使用新内容，不需要重新打包或重启网站。支持 Markdown，预览中的危险内容会被自动清理。',
        openGuide: '打开教程页',
        currentVersion: '当前版本：第 {version} 版',
        databaseContent: '数据库内容',
        bundledContent: '程序内置教程',
        updatedAt: '更新时间：{date}',
        markdown: '教程内容（Markdown）',
        chapters: '章节',
        chapterCount: '共 {count} 章',
        editingChapter: '正在编辑：{title}',
        anchorHint: '本章锚点为 #{slug}，改标题不会改变它，已分享的链接依然有效。',
        ...
        deleteChapterMessage: '确定删除锚点为 #{slug} 的这一章吗？发布后该章节会从教程页消失，指向它的链接会失效。',
        importBundled: '引入内置教程中缺少的 {count} 章',
        chapterTooLarge: '当前章节超过 64 KiB，请删减后再保存。',
        assets: '附件',
        assetUpload: '上传文件',
        assetUploading: '正在上传...',
        assetHint: '也可以直接拖入或粘贴。单文件最大 32 MiB；图片会内嵌显示，其他类型生成下载链接。',
        assetQuota: '已用 {used}',
        assetInsert: '插入',
        assetCopy: '复制链接',
        assetCopied: '链接已复制',
        assetCopyFailed: '复制失败',
        assetDelete: '删除',
        assetDeleteTitle: '删除附件',
        assetDeleteMessage: '确定删除「{name}」吗？教程里引用它的地方会失效，需要你自己改掉。',
        assetDeleted: '附件已删除',
        assetDeleteFailed: '附件删除失败',
        assetEmpty: '还没有上传过文件。',
        assetEmptyChapter: '请先选择一个章节再上传。',
        assetNeedChapter: '请先选择一个章节，上传后会把引用插入该章节。',
        assetUploadFailed: '「{name}」上传失败',
        assetLoadFailed: '附件列表加载失败',
        preview: '实时预览（全部章节）',
        sanitizedPreview: '已安全处理',
        placeholder: '在这里填写本章内容，首行请保留 ## 标题...',
        size: '{size}',
        tooLarge: '全部章节合计超过 256 KiB，请删减后再保存。',
        useBundled: '恢复内置教程',
        unsaved: '有尚未发布的修改',
        saving: '正在发布...',
        publish: '发布教程',
        historyTitle: '版本记录',
        historyDescription: '保留最近 20 次发布。恢复旧版本时会生成一个新版本，原记录不会被删除。',
        noHistory: '还没有发布记录。当前正在使用程序内置教程。',
        revision: '第 {version} 版',
        current: '当前',
        restore: '恢复此版本',
        resetTitle: '恢复程序内置教程',
        resetMessage: '确定恢复吗？当前数据库教程会保留在版本记录中，以后仍可恢复。',
        restoreTitle: '恢复旧版本',
        restoreMessage: '确定把第 {version} 版重新发布为当前教程吗？',
        loadFailed: '教程加载失败',
        saveFailed: '教程保存失败',
        conflict: '教程已被另一个管理页面修改，已为你重新加载最新版本。',
        publishSuccess: '教程已发布，刷新 /guide 即可看到',
        resetSuccess: '已恢复程序内置教程',
      ...
      registration: {
        ...
        emailSuffixWhitelistHint:
          "仅允许使用指定域名的邮箱注册用户；留空则不限制（例如 {'@'}qq.com, {'@'}gmail.com, *.edu.cn）",
        ...
        emailDomainQuotaHint:
          '开启后，白名单非空时，其他可注册主域名各限注册一个用户；关闭时非白名单域名直接拒绝注册。白名单为空时本开关无效果',
      ...
      security: {
        ...
        stepUpEnableRequiresTotp: '开启敏感操作二次验证前，请先在个人设置中为当前用户启用 2FA (TOTP)。',
      ...
      panelRateLimit: {
        ...
        proxySafeNote: '登录后的接口按「用户」维度计数，与来源 IP 无关——反向代理、NAT 共享出口等场景不会被误拦截；公开接口按真实客户端 IP 计数，回环与内网地址（反代内部转发地址）会自动跳过。',
        enabled: '启用面板接口限流',
        enabledHint: '对登录后的面板接口按用户限流；超出阈值返回 429，窗口重置后自动恢复。',
        userRpm: '每用户请求上限',
        userRpmHint: '单个用户每分钟允许的面板 API 请求总数，正常页面操作远达不到该阈值；0 表示不限制。',
        heavyRpm: '重查询请求上限',
        heavyRpmHint: '单个用户每分钟允许的用量/仪表盘等聚合统计查询次数（这类请求对数据库压力最大）；0 表示不限制。',
        publicIpRpm: '公开接口每 IP 上限',
        publicIpRpmHint: '无需登录的公开接口（如站点公开设置）每个真实客户端 IP 每分钟的请求上限；0 表示不限制。',
        perMinute: '次/分钟',
        exemptAdmin: '管理员豁免',
        exemptAdminHint: '开启后管理员用户不受面板限流约束，避免批量运维操作被误拦。',
        saved: '面板接口限流配置已保存',
      ...
      tencentCaptcha: {
        title: '腾讯天御验证码',
        description: '为登录、注册及第三方登录注册流程提供滑动验证码保护',
        enable: '启用腾讯天御验证码',
      ...
      streamTimeout: {
        title: '流超时处理',
        description: '配置上游响应超时时的账号处理策略，避免问题账号持续被选中',
        enabled: '启用流超时处理',
        enabledHint: '当上游响应超时时，自动处理问题账号',
      ...
      authSourceDefaults: {
        ...
        enabledHint: '以下默认值会在该来源注册新用户时发放；首次绑定时授权仅作用于已有用户绑定该来源。',
        sources: {
          email: {
            title: '邮箱注册',
        ...
        grantOnFirstBindHint: '已有用户首次绑定该来源时发放默认权益。',
