export default {
    accounts: {
      ...
      openai: {
        ...
        outboundCookieWS: 'Cookie WS（默认）',
        outboundCookieWSDesc: '保持现网 Astra Cookie websocket / HTTP 回退。不写 BPS extra。',
        ...
        excelBPSDesc: '使用本账号已有的 ChatGPT OAuth 凭据，经 Excel 接口转发 Responses 请求。与 Cookie WS 二选一；关闭后恢复默认 Cookie WS。',
        excelBPSAllModels: '对所有模型启用（兼容原设置）',
        excelBPSModels: '勾选使用 Excel / BPS 的模型',
        excelBPSAstraOnly: '仅选 Astra',
        excelBPSModelsHint: '按账号映射后的模型名称匹配。仅勾选模型走 Excel / BPS；未选模型保留 Cookie WS / HTTP 回退。不勾选任何模型时不使用 BPS。',
        excelBPSNotice: '保存后新开 Codex 会话。所选模型强制 HTTP/SSE，忽略 WS mode 与门票注入；仅支持 Responses、客户端工具和 HTTPS 图片链接，不支持 base64 图片（除非开启图片中转）。max / ultra 按 xhigh 发送，模型权限以上游为准。',
        ...
        longContextBillingDesc: '默认开启。prompt（输入+缓存）超过 272K 时按官方长上下文计费：输入/缓存 2 倍、输出 1.5 倍。',
        ...
        codexCookieVerifiedWS: '已验证 WS {count} / 最少 {minimum}',
        codexCookieRefreshAt: '计划刷新：{time}',
        codexCookieCapturedAt: '获取时间：{time}',
        codexCookieExpiresAt: '过期时间：{time}',
        codexCookieGroupCounts: '当前进程已验证 Cookie 组 {ready}/{total} · 有效期内 Cookie 组 {valid}/{total}',
        codexCookieSlot: '第 {number} 组',
        codexCookieSlotVerified: 'Cookie 已验证',
        codexCookieSlotUnverified: 'Cookie 待验证',
        codexCookieSlotSockets: '已验证 WS {count}/{total}',
        codexCookieRefreshDiagnostic: 'Cookie 刷新',
        codexCookieWarmupDiagnostic: 'WS 恢复',
        codexCookieAttempts: '尝试次数：{count}',
        codexCookieLastAttempt: '上次尝试：{time}',
        codexCookieLastSuccess: '上次成功：{time}',
        codexCookieLastFailure: '上次失败：{time}',
        codexCookieNextAttempt: '最早可重试：{time}',
        codexCookieRetryHint: '重试时间表示最早允许重试的时间，实际执行还受后台检查轮次和账号可调度状态影响。',
        codexCookiePauseReason: '暂停原因：{reason}',
        codexCookieRecovery: {
          ready: 'WS {count}/{total} · 就绪',
          partial: 'WS {count}/{total} · 部分可用',
          recovering: 'WS {count}/{total} · 恢复中',
          paused: 'WS {count}/{total} · 已暂停',
          unavailable: 'WS —/{total} · 状态不可读取'
        },
        codexCookiePhase: {
          idle: '无进行中任务',
          no_record: '暂无记录',
          ready: '就绪',
          waiting: '等待检查',
          restoring: '恢复已有 Cookie',
          harvesting: '获取 Cookie',
          validating: '验证 Cookie',
          warming: '建立并验证 WS',
          backoff: '等待重试',
          paused: '已暂停',
          unavailable: '状态不可读取'
        ...
        codexCookieSkipReason: {
          not_cookie_account: '账号不适用 Cookie WS',
          not_active: '账号未启用',
          manual_unschedulable: '账号已手动停止调度',
          expired: '账号已过期',
          rate_limited: '账号限流中',
          overloaded: '上游过载冷却中',
          temporarily_unschedulable: '账号暂时不可调度',
