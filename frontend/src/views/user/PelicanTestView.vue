  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <div v-else class="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <article v-for="item in items" :key="item.id" class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-dark-700 dark:bg-dark-800">
...
onMounted(async () => {
  try {
    const page = await pelicanTestsAPI.list()
