...
type GroupQuery struct {
	...
	withSecurityPolicyKeywords *SecurityPolicyKeywordQuery
...
// QuerySecurityPolicyKeywords chains the current query on the "security_policy_keywords" edge.
func (_q *GroupQuery) QuerySecurityPolicyKeywords() *SecurityPolicyKeywordQuery {
	query := (&SecurityPolicyKeywordClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(group.Table, group.FieldID, selector),
			sqlgraph.To(securitypolicykeyword.Table, securitypolicykeyword.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, true, group.SecurityPolicyKeywordsTable, group.SecurityPolicyKeywordsColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withSecurityPolicyKeywords: _q.withSecurityPolicyKeywords.Clone(),
...
// WithSecurityPolicyKeywords tells the query-builder to eager-load the nodes that are connected to
// the "security_policy_keywords" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithSecurityPolicyKeywords(opts ...func(*SecurityPolicyKeywordQuery)) *GroupQuery {
	query := (&SecurityPolicyKeywordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSecurityPolicyKeywords = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	var (
		...
		loadedTypes = [9]bool{
			...
			_q.withSecurityPolicyKeywords != nil,
	...
	if query := _q.withSecurityPolicyKeywords; query != nil {
		if err := _q.loadSecurityPolicyKeywords(ctx, query, nodes,
			func(n *Group) { n.Edges.SecurityPolicyKeywords = []*SecurityPolicyKeyword{} },
			func(n *Group, e *SecurityPolicyKeyword) {
				n.Edges.SecurityPolicyKeywords = append(n.Edges.SecurityPolicyKeywords, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadSecurityPolicyKeywords(ctx context.Context, query *SecurityPolicyKeywordQuery, nodes []*Group, init func(*Group), assign func(*Group, *SecurityPolicyKeyword)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(securitypolicykeyword.FieldGroupID)
	}
	query.Where(predicate.SecurityPolicyKeyword(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.SecurityPolicyKeywordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
