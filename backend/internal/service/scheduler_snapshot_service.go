...
func (s *SchedulerSnapshotService) ListSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, error) {
...
func (s *SchedulerSnapshotService) handleBulkAccountEvent(ctx context.Context, payload map[string]any, seen map[batchSeenKey]struct{}) error {
	...
	for _, account := range accounts {
		...
		case PlatformAnthropic, PlatformOpenAI, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek:
...
func schedulerSnapshotPlatforms() [6]string {
	return [6]string{PlatformAnthropic, PlatformOpenAI, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek}
}
...
func schedulerCanonicalBuckets(groupID int64) []SchedulerBucket {
	buckets := make([]SchedulerBucket, 0, 2*len(schedulerSnapshotPlatforms()))
	for _, platform := range schedulerSnapshotPlatforms() {
...
func (s *SchedulerSnapshotService) rebuildByGroupIDs(ctx context.Context, groupIDs []int64, reason string, seen map[batchSeenKey]struct{}) error {
	...
	buckets := make([]SchedulerBucket, 0, len(groupIDs)*2*len(schedulerSnapshotPlatforms()))
	for _, platform := range schedulerSnapshotPlatforms() {
...
func (s *SchedulerSnapshotService) loadAccountsForRebuild(
	...
	if queries == nil || !cacheable {
		return s.loadAccountsFromDB(ctx, bucket)
	}
	...
	accounts, err := s.loadAccountsFromDB(ctx, bucket)
	if err != nil {
		return nil, err
	}
