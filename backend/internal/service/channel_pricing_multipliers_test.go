...
func TestCalculateTokenCostContextTierEnablement(t *testing.T) {
	...
	t.Run("group enabled and account disabled uses base tier", func(t *testing.T) {
	...
	t.Run("account enabled and group disabled uses base tier", func(t *testing.T) {
		resolved.longContextPricingEnabled = false
		accountEnabled := true
		cost, err := service.calculateTokenCost(resolved, CostInput{
			Model: "custom", Tokens: tokens, RateMultiplier: 1, Resolver: resolver,
	...
	t.Run("group and account enabled uses interval", func(t *testing.T) {
		resolved.longContextPricingEnabled = true
		accountEnabled := true
		cost, err := service.calculateTokenCost(resolved, CostInput{
			Model: "custom", Tokens: tokens, RateMultiplier: 1, Resolver: resolver,
	...
	t.Run("group enabled without account gate uses interval", func(t *testing.T) {
		resolved.longContextPricingEnabled = true
		cost, err := service.calculateTokenCost(resolved, CostInput{
			Model: "custom", Tokens: tokens, RateMultiplier: 1, Resolver: resolver,
		})
