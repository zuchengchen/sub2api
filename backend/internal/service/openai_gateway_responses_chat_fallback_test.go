...
func TestForwardResponses_ForceChatCompletionsRoutesNonStreamingToChatCompletions(t *testing.T) {
	...
	result, err := svc.Forward(responsesReasoningTestContext(42), c, forceChatResponsesFallbackAccount(), body)
...
func responsesReasoningTestContext(userID int64) context.Context {
...
func TestForwardResponses_ChatFallbackRestoresReasoningFromCache(t *testing.T) {
	...
	cache := &reasoningRecordingCache{
		getResp: map[string]string{"user:42:item_enc1": "cached thinking"},
	}
	svc := &OpenAIGatewayService{
		cfg:          rawChatCompletionsTestConfig(),
		httpUpstream: upstream,
	...
	require.Equal(t, "plain thinking", cache.snapshotSets()["user:42:item_plain"])
}

func TestResponsesReasoningCacheIsScopedByAuthenticatedUser(t *testing.T) {
	scope42 := responsesReasoningCacheScope(responsesReasoningTestContext(42))
	scope43 := responsesReasoningCacheScope(responsesReasoningTestContext(43))

	require.Equal(t, "user:42:item_shared", responsesReasoningCacheItemKey(scope42, "item_shared"))
	require.Equal(t, "user:43:item_shared", responsesReasoningCacheItemKey(scope43, "item_shared"))
	require.NotEqual(t,
		responsesReasoningCacheItemKey(scope42, "item_shared"),
		responsesReasoningCacheItemKey(scope43, "item_shared"),
	)
	require.Empty(t, responsesReasoningCacheScope(context.Background()))
	require.Empty(t, responsesReasoningCacheItemKey("", "item_shared"))

	cache := &reasoningRecordingCache{getResp: map[string]string{
		"user:42:item_shared": "user 42 reasoning",
		"user:43:item_shared": "user 43 reasoning",
	}}
	svc := &OpenAIGatewayService{cache: cache}
	require.Equal(t, "user 42 reasoning", svc.reasoningContentByID(scope42, "item_shared"))
	require.Equal(t, "user 43 reasoning", svc.reasoningContentByID(scope43, "item_shared"))
	require.Empty(t, svc.reasoningContentByID("", "item_shared"))
}
