...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformComposite {
		...
		if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
			...
			if len(source) == 0 {
				source = filterAccessible(defaultModelIDsForPlatform(service.PlatformComposite))
			}
	...
	if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
		source := modelListingSource(platform, availableModels, filterAccessible(defaultModelIDsForPlatform(platform)))
...
func (h *GatewayHandler) compositeAvailableModels(ctx context.Context, groupID *int64) []string {
	...
	for _, platform := range []string{service.PlatformAnthropic, service.PlatformOpenAI, service.PlatformGrok, service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformMiniMax} {
...
func defaultModelIDsForPlatform(platform string) []string {
	...
	case service.PlatformComposite:
		ids := make([]string, 0)
		seen := make(map[string]struct{})
		for _, concretePlatform := range []string{service.PlatformAnthropic, service.PlatformOpenAI, service.PlatformGrok, service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformMiniMax} {
...
func (h *GatewayHandler) handleFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, platform string, streamStarted bool) {
	...
	if msg, ok := service.UsagePolicyClientError(responseBody); ok {
