...
func (h *OpenAIGatewayHandler) Embeddings(c *gin.Context) {
	...
	if decision := h.checkContentModeration(c, reqLog, apiKey, subject, "openai_embeddings", reqModel, body); decision != nil && !decision.Allowed {
	...
	for {
		...
		if err != nil {
			...
			if lastFailoverErr != nil {
				...
				h.errorResponse(c, http.StatusBadGateway, "api_error", userfacing.UpstreamRequestFailed)
			}
			return
		}
