...
// key ring itself remains outside the repository and database.
type ContentModerationArchiveConfig struct {
...
type GatewayOpenAIWSSchedulerScoreWeights struct {
	...
	// Reset 倾向「7 天额度窗口最早重置」的账号（use-it-or-lose-it）。
	// 剩余重置时间越短的账号得分越高，避免周期到期时额度浪费。默认 3.0。
...
func setDefaults() {
	...
	viper.SetDefault("gateway.openai_ws.scheduler_score_weights.reset", 3.0)
	...
	viper.SetDefault("gateway.image_nonstream_keepalive_interval", 10)
