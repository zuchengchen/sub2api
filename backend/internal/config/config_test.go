...
func TestLoadDefaultOpenAIWSConfig(t *testing.T) {
	...
	if cfg.Gateway.OpenAIWS.SchedulerScoreWeights.Reset != 3 {
		t.Fatalf("Gateway.OpenAIWS.SchedulerScoreWeights.Reset = %v, want 3", cfg.Gateway.OpenAIWS.SchedulerScoreWeights.Reset)
	}
...
func TestLoad_DefaultGatewayImageStreamConfig(t *testing.T) {
	...
	if cfg.Gateway.ImageNonstreamKeepaliveInterval != 10 {
		t.Fatalf("image_nonstream_keepalive_interval = %d, want 10", cfg.Gateway.ImageNonstreamKeepaliveInterval)
	}
