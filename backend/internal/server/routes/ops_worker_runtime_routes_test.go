...
func TestWorkerRuntimeStatusRouteRequiresAdminAuthentication(t *testing.T) {
	...
	path := "/api/v1/admin/ops/workers/status"
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NotContains(t, recorder.Body.String(), "payload")
		})
	}
}
