...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	authHandler := handler.NewAuthHandler(configConfig, authService, userService, settingService, redeemService, totpService, userAttributeService)
	...
	tokenCache := repository.NewTokenCache(redisClient)
	compositeTokenCacheInvalidator := service.NewCompositeTokenCacheInvalidator(tokenCache)
	...
	oAuthRefreshAPI := service.ProvideOAuthRefreshAPI(accountRepository, tokenCache)
	claudeTokenProvider := service.ProvideClaudeTokenProvider(accountRepository, tokenCache, oAuthService, oAuthRefreshAPI)
	...
	openAITokenProvider := service.ProvideOpenAITokenProvider(accountRepository, tokenCache, openAIOAuthService, oAuthRefreshAPI)
	...
	grokTokenProvider := service.ProvideGrokTokenProvider(accountRepository, tokenCache, grokOAuthService, oAuthRefreshAPI, tempUnschedCache)
	...
	antiDegradeService := service.ProvideAntiDegradeService(adminService)
	...
	contentModerationService, err := service.ProvideContentModerationService(settingRepository, contentModerationRepository, contentModerationHashCache, groupRepository, userRepository, proxyRepository, apiKeyAuthCacheInvalidator, emailService, apiKeyRepository, configConfig)
	if err != nil {
		return nil, err
	}
	securityPolicyRepository := repository.NewSecurityPolicyRepository(client, db)
	securityPolicySessionStore := repository.NewSecurityPolicySessionStore(redisClient)
	securityPolicyLogStore := repository.ProvideSecurityPolicyLogStore(contentModerationRepository)
	securityPolicyService := service.NewSecurityPolicyService(contentModerationService, securityPolicyRepository, securityPolicySessionStore, securityPolicyLogStore, settingRepository, groupRepository, emailService)
	...
	tokenRefreshService := service.ProvideTokenRefreshService(accountRepository, oAuthService, openAIOAuthService, grokOAuthService, compositeTokenCacheInvalidator, schedulerCache, configConfig, tempUnschedCache, privacyClientFactory, proxyRepository, oAuthRefreshAPI, openAIGatewayService)
	...
	guideAssetStore := handler.ProvideGuideAssetStore(settingService)
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, guideAssetStore)
	billingOutboxRepository := repository.NewBillingOutboxRepository(db)
	billingOutboxWorker := service.ProvideBillingOutboxWorker(billingOutboxRepository, usageBillingRepository, apiKeyService, gatewayService, openAIGatewayService)
	...
	opsHandler := handler.ProvideOpsHandler(opsService, billingOutboxWorker, runtime)
	...
	usagePolicyService := service.ProvideUsagePolicyService(usagePolicyRepository, settingRepository, apiKeyAuthCacheInvalidator, opsService, contentModerationService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
